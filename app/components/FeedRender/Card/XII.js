...
const DimzouFeedCardXII = (props) => {
  ...
  if (!data) {
    return (
      <article
        style={layoutVars}
        role="presentation"
        className={classNames('DimzouFeedCard DimzouFeedCard_XII', {
  ...
  const renderData = context.mapData
    ? context.mapData(data, index, DimzouFeedCardXII.code)
    : data;
  const {
    cover,
    title,
...
DimzouFeedCardXII.propTypes = cardPropTypes;
DimzouFeedCardXII.defaultProps = {
  showAvatar: true,
  avatarRender: defaultAvatarRender,
};

DimzouFeedCardXII.code = 'CardXII';

export default DimzouFeedCardXII;
