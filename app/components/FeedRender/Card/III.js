...
const DimzouFeedCardIII = (props) => {
  ...
  if (!data) {
    return (
      <article
        style={layoutVars}
        role="presentation"
        className={classNames('DimzouFeedCard DimzouFeedCard_III', {
  ...
  const renderData = context.mapData
    ? context.mapData(data, index, DimzouFeedCardIII.code)
    : data;
  const {
    cover,
    title,
...
DimzouFeedCardIII.propTypes = cardPropTypes;
DimzouFeedCardIII.defaultProps = {
  showAvatar: true,
  avatarRender: defaultAvatarRender,
};
DimzouFeedCardIII.code = 'CardIII';

export default DimzouFeedCardIII;
