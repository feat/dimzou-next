...
function NodeNavAnchor() {
  ...
  const nodeIndex = nodes.findIndex((item) => String(item.id) === workspace.nodeId);
  const next = nodeIndex > -1 ? nodes[nodeIndex + 1] : null;
  const nodeState = useSelector((state) => next ? selectNodeState(state, { nodeId: next.id }) : null);
  useEffect(() => {
    const handleScroll = () => {
      ...
      // prefetch data;
      if (
        box.top - viewportHeight < 100 &&
        box.top - viewportHeight > 0 
      ) {
        if (!nodeState) {
          const params = {
          ...
          dispatch(asyncFetchNodeEditInfo(params))
          // .then(() => {
          //   if (autoRedirect) {
          //     Router.push({
