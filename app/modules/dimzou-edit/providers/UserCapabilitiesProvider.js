...
function UserCapabilitiesProvider(props) {
  ...
  const collaborators = get(nodeState, 'data.collaborators');
  const editPermission = get(nodeState, 'data.permission');
  ...
  const userCapabilities = useMemo(() => {
    ...
    const userRole = getUserRole(currentUserId, collaborators, editPermission);
    const published = bundleState.data && bundleState.data.status === BUNDLE_STATUS_PUBLISHED;
    ...
    //   if (isOriginBundle(bundleState.data) && nodeState.data.type === NODE_TYPE_CHAPTER) {
    //     canEdit = false;
    //   }
    // }
    ...
    return {
      ...
      canChangeTemplate: features.canChangeTemplate && nodeState.data.type === NODE_TYPE_CHAPTER && userIsOwner,
      canPublish: userIsOwner && !published,
      canEdit,
      canAppendContent: features.canAppendContent,
    };
  }, [
    collaborators,
    editPermission,
